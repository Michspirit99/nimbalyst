  ...
  it('runs concurrent large-workspace workloads and records metrics', async () => {
    ...
    const { result, metrics } = await measureEventLoop(async () => {
      return Promise.all(fixtures.map(async (fixture) => {
        ...
        // Model the production boundary: bounded work followed by an event-loop
        // yield, repeated for each workspace's diff/query batch.
        for (let batch = 0; batch < 10; batch += 1) {
          // Touch the generated fixture so this is not an empty workload.
    ...
    // Keep the result shape visible in CI output without enforcing hardware-
    // dependent thresholds. The benchmark is intended for before/after runs.
