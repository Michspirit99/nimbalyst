...
/**
 * Helper: normalize an assertion value - used when checking expected results from functions
 * that may return platform-specific path formats (Windows vs Unix).
 */
function normalizeExpected(expected: string | null): string | null {
  return expected ? normalizePath(expected) : null;
}

describe('resolveProjectPath', () => {
  it('returns the same path for regular workspaces', () => {
    expect(normalizePath(resolveProjectPath('/path/to/project'))).toBe('/path/to/project');
    expect(normalizePath(resolveProjectPath('/Users/dev/my-app'))).toBe('/Users/dev/my-app');
    expect(normalizePath(resolveProjectPath('/home/user/code/myrepo'))).toBe('/home/user/code/myrepo');
  });

  it('resolves worktree paths to parent project', () => {
    expect(normalizePath(resolveProjectPath('/path/to/project_worktrees/swift-falcon'))).toBe('/path/to/project');
    expect(normalizePath(resolveProjectPath('/Users/dev/my-app_worktrees/brave-eagle'))).toBe('/Users/dev/my-app');
    expect(normalizePath(resolveProjectPath('/home/user/code/myrepo_worktrees/test-123'))).toBe('/home/user/code/myrepo');
  });
...
it('resolves NESTED worktree paths (branch-style names) to parent project', () => {
      ...
      expect(normalizePath(resolveProjectPath('/path/to/project_worktrees/feature/my-branch'))).toBe('/path/to/project');
      expect(normalizePath(resolveProjectPath('/Users/dev/Belegify_worktrees/feature/assign-people-on-receipts'))).toBe('/Users/dev/Belegify');
      expect(normalizePath(resolveProjectPath('/Users/dev/nimbalyst_worktrees/improvement/fix-askme-for-worktrees'))).toBe('/Users/dev/nimbalyst');
    });

it('resolves a subfolder inside a nested worktree to the parent project', () => {
      expect(normalizePath(resolveProjectPath('/path/to/project_worktrees/feature/my-branch/packages/electron'))).toBe('/path/to/project');
    });

    it('resolves nested worktrees with underscore project names', () => {
      expect(normalizePath(resolveProjectPath('/path/to/my_cool_project_worktrees/feature/x'))).toBe('/path/to/my_cool_project');
    });
    ...
    it('does not resolve a plain subfolder of a non-worktree project (the permission cascade handles that)', () => {
      expect(normalizePath(resolveProjectPath('/path/to/project/packages/electron'))).toBe('/path/to/project/packages/electron');
    });
    ...
    it('does not match paths that just contain _worktrees in the middle', () => {
      ...
      expect(normalizePath(resolveProjectPath('/path/to/project_worktrees_backup/folder'))).toBe('/path/to/project_worktrees_backup/folder');
    });

it('handles empty and null-ish inputs gracefully', () => {
      expect(normalizePath(resolveProjectPath(''))).toBe('');
    ...
    it('handles complex project names with underscores', () => {
      expect(normalizePath(resolveProjectPath('/path/to/my_cool_project_worktrees/branch-1'))).toBe('/path/to/my_cool_project');
    });
    ...
    it('handles Windows-style paths', () => {
      ...
      expect(normalizePath(resolveProjectPath('C:/Users/dev/project_worktrees/feature'))).toBe('C:/Users/dev/project');
    });
  });
...
describe('findNearestAncestor', () => {
  const trusted = new Set([path.normalize('/path/to/project')]);
  const pred = (dir: string) => trusted.has(dir);

  it('returns the start path itself when it matches', () => {
    expect(normalizeExpected(findNearestAncestor('/path/to/project', pred))).toBe('/path/to/project');
  });

it('walks up to the nearest matching ancestor (subfolder cascade)', () => {
      expect(normalizeExpected(findNearestAncestor('/path/to/project/packages/electron', pred))).toBe('/path/to/project');
      expect(normalizeExpected(findNearestAncestor('/path/to/project/src', pred))).toBe('/path/to/project');
    });
...
it('returns the most specific matching ancestor when several match', () => {
      const t2 = new Set([path.normalize('/a'), path.normalize('/a/b/c')]);
      expect(normalizeExpected(findNearestAncestor('/a/b/c/d', (d) => t2.has(d)))).toBe('/a/b/c');
    });

    it('handles empty input and trailing slashes', () => {
      expect(findNearestAncestor('', pred)).toBe(null);
      expect(normalizeExpected(findNearestAncestor('/path/to/project/packages/', pred))).toBe('/path/to/project');
    });
...
describe('stopAt boundary', () => {
      it('still returns a match found at or below the boundary', () => {
        ...
        expect(normalizeExpected(findNearestAncestor('/path/to/project/src', pred, '/path/to/project'))).toBe('/path/to/project');
      });
      ...
      it('does NOT climb past the boundary to a higher match', () => {
        ...
        const t = new Set([path.normalize('/root')]);
      ...
      it('returns the nearer match even when a farther one also matches', () => {
        const t = new Set([path.normalize('/root'), path.normalize('/root/child')]);
        const p = (d: string) => t.has(d);
        expect(normalizeExpected(findNearestAncestor('/root/child/leaf', p, '/root/child'))).toBe('/root/child');
      });
    });
  });
